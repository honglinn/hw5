...
*/
void _adjustHeap(DynArr *heap, int max, int pos)            //modified**********************/
{
   /* FIXME */
	...
	int minchild;
	if(rightchild<max){
		minchild = _smallerIndexHeap(heap,leftchild,rightchild);
		if(compare(getDynArr(heap,pos),getDynArr(heap,minchild))==1){  
			swapDynArr(heap,pos,minchild);
			_adjustHeap(heap, max, minchild);
		}	
...
void removeMinHeap(DynArr *heap)        
{
   /* FIXME */
	...
	if(compare(getDynArr(heap,0),getDynArr(heap,sizeDynArr(heap)-1))==-1)
	{
